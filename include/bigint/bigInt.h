...
// ================================================
static const std::size_t BIG_INT_MAJOR_VERSION = 7;
static const std::size_t BIG_INT_MINOR_VERSION = 1;
// ================================================
...
struct BigInt
{
	...
	BOOST_STATIC_ASSERT (sizeof(std::size_t) <= sizeof(Ulong));
	// ---------------------------------------------
    ...
    static_assert (SUB_BASE2 == 999999999, "(SUB_BASE2 == 999999999) failed");
    static_assert (!(BASE2 >= MAX_UNIT_VALUE), "(!(BASE2 >= MAX_UNIT_VALUE)) failed");
    static_assert ((BASE1 * (BASE2/BASE1 + 1) < MAX_UNIT_VALUE), "(BASE1 * (BASE2/BASE1 + 1) < MAX_UNIT_VALUE) failed");
    static_assert (!(BASE2 != (SUB_BASE2 + 1)), "(!(BASE2 != (SUB_BASE2 + 1))) failed");
    static_assert (OVER_BASE2 > SUB_BASE2, "(OVER_BASE2 > SUB_BASE2) failed");
    ...
    static_assert(
							...
							"((sizeof(Ulong) == 4) && ((ULONG_MSB == static_cast<Ulong>(0x80000000)))|| (sizeof(Ulong) == 8) && ((ULONG_MSB == ((static_cast<Ulong>(0x80000000) << 31) << 1)))) failed"
                       );
	...
	//  sizeof (std::size_t) == 8, but sizeof (unsigned long) == 4
	//  If ASSERTION() failed then "typedef Ulong" should be changed
	static_assert (sizeof(std::size_t) <= sizeof(Ulong), "(sizeof(std::size_t) <= sizeof(Ulong)) failed");
	// ---------------------------------------------
...
protected :
    ...
    std::vector<Ulong>                  m_units;      // BigHexDigit is Ulong
...
public :
	...
	// -------------------------
	// toUlong () has sense only while m_units.size() == 1
    Ulong toUlong () const;
...
public :
	...
	static Rossi  pow(const Rossi& i_base, const Ulong i_exp, const bool i_iterative = true);
...
private:
	...
	static void testPow(std::ostream& o_stream, const BigInt::Rossi& i_base, const BigInt::Ulong i_exp);

	static void testAssertDouble(std::ostream& o_stream, const BigInt::Ulong i_ulongValue);
	static void testAssertPow(std::ostream& o_stream, const BigInt::Ulong i_base, const BigInt::Ulong i_exp);
